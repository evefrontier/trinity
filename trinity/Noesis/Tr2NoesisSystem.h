...
// process-wide ApplicationResources. An empty path, missing XAML or a root that is
// not a ResourceDictionary is reported on the Noesis log channel and leaves the
// existing resources in place. RefreshDefaultStyles is called on success so live
// views pick up implicit styles and DynamicResource keys.
